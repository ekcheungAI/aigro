...
3. 設定 Edge Function secrets：MiniMax、OpenAI、Firecrawl、YouTubeTranscript.dev、Turnstile、worker secret、Resend 與 booking webhook secret。
4. 在 Vault 加 `project_url`、`knowledge_worker_secret`、`booking_webhook_secret`，確保 pg_cron／pg_net 可以呼叫 deployed functions。
5. Deploy `ask-answer`、`knowledge-worker`、`booking-notify`，並保持 `supabase/config.toml` 的 JWT 設定。
6. 設定 private Storage bucket policy，逐位導師先只開 `cms_ingestion_enabled`；完成 verified question evaluation 後再開 `rag_enabled`，最後開 `booking_enabled`。

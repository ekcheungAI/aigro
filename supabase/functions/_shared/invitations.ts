...
export function parseInvitationId(input: unknown): InvitationIdParseResult {
  ...
  return UUID_PATTERN.test(value)
    ? { ok: true, value }
    : { ok: false, code: "invalid_invitation_id", error: "Invalid invitation id" };
}

export function parseInvitationInput(input: unknown): InvitationParseResult {
  if (!input || typeof input !== "object") return invalid("invalid_request", "Invalid request");
  const record = input as Record<string, unknown>;
  const email = typeof record.email === "string" ? record.email.trim().toLowerCase() : "";
  const name = typeof record.name === "string" ? record.name.trim() : "";
  const memberClass = typeof record.memberClass === "string" ? record.memberClass : "founding";
  ...
  if (!name || name.length > 80) return invalid("invalid_name", "姓名必須為 1 至 80 個字元");
  if (!MEMBER_CLASSES.has(memberClass)) return invalid("invalid_member_class", "Invalid member class");
...
<html lang="zh-HK">
      ...
      <tr><td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#FFFFFF;border:1px solid #D9DDD6">
          ...
          <tr><td style="padding:36px 32px">
            <p style="margin:0 0 12px;color:#1F7A06;font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase">Private invitation</p>
            ...
            <p style="margin:0;font-size:13px;line-height:1.7;color:#676B65">為保障帳戶安全，邀請連結會按 Supabase Auth 設定自動失效。如果按鈕未能開啟，請複製以下連結：</p>
            <p style="margin:8px 0 0;font-size:12px;line-height:1.6;word-break:break-all;color:#1F7A06">${safeActionUrl}</p>
          </td></tr>
          <tr><td style="padding:20px 32px;border-top:1px solid #D9DDD6;font-size:12px;line-height:1.6;color:#676B65">AIGRO · AI + Growth · Hong Kong<br>如果你唔認識邀請人，可以忽略呢封郵件。</td></tr>
        </table>
      </td></tr>
    </table>
  </body>
  ...
  const text = `${recipientName.trim()}，歡迎加入 AIGRO\n\n${inviterName.trim()} 邀請你加入香港 AI・增長・商業情報平台 AIGRO。${messageText}\n\n接受邀請並設定密碼：\n${actionUrl}\n\n如果你唔認識邀請人，可以忽略呢封郵件。`;
